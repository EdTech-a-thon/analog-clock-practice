...
There is no GitHub Issues workflow for this repo. The GitHub remote exists to
host the code and to run the publish pull request workflow; work is tracked
here, in the repo, in git.
...
The `triage` skill is not installed in this repo, so there is no label mapping
file. Skills that name a triage role should write that role verbatim on the
`Status:` line — `ready-for-agent` for a fully specified item ready for an
implementing agent. If `triage` is ever installed, run
`/setup-matt-pocock-skills` again to generate `docs/agents/triage-labels.md`.
